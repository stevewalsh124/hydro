...
if(PDF) pdf(paste0("pdf/hydro-SA-",des,if(des=="FF"){n_ff},if(des=="unif"){paste0("-",m)},"_new32.pdf"))

# generate the parameter variations for the SA
# load("avg_mean_hydro.rda")
load("rda/mainEffs_real_new32.rda")
avg_mean_MEs <- rowMeans(mainEffs_real)


nv = dim(mainEffs_real)[2]
...
write(t(des01),file='hydro_design_out_new32.txt',ncol=p,sep=' ')
...
write(t(eta),file='eta-hydro_new32.txt',ncol=nruns)
...
if(PDF) dev.off()
save.image(paste0("/projects/precipit/hydro/hydro-SA-",des,if(des=="FF"){n_ff},if(des=="unif"){paste0("-",m)},"_new32.rda"))
